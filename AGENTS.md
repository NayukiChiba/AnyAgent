...
每条 commit 必须同时包含标题和正文。只有 `type` 和 `scope` 使用英文，标题描述和正文均使用中文；文件名、命令和技术标识保留原样。遵循 [Conventional Commits](https://www.conventionalcommits.org/zh-hans/)：

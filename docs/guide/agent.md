...
Agent 回复支持标题、加粗、斜体、删除线、列表、引用、表格、链接、图片和代码块。代码块显示语言、语法高亮与“复制代码”；未知语言按原文展示。长代码和表格在自己的区域内横向滚动，适配移动端。

流式输出中的未闭合代码块也能展示，最终回复会更新并由后端保存原始 Markdown；刷新或重启后重新渲染。用户输入按原文显示。渲染禁用原始 HTML 并清洗结果，外部链接在新页面打开；复制失败时可手动选中代码复制。

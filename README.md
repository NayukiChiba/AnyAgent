...
端口确认可用后，终端会输出 AnyAgent Banner、前端状态以及聊天页、设置页和 API
文档地址；监听 `0.0.0.0` 时还会列出局域网访问地址。
...
Agent 回复支持 Markdown 标题、列表、引用、表格、链接和图片；代码块提供语法高亮与复制，流式回复边到达边渲染。用户输入保持原文显示，历史仍保存原始文本。

## 网页重启

通过 `main.py` 启动后，设置页面下方提供“重启服务”。先保存或撤销草稿，再确认
重启；服务会清理连接、日志及子进程，然后读取最新配置并启动新实例。Windows 在
当前受控进程内重建服务，其他平台重新执行绝对入口，已保存的会话保留，进行中的
